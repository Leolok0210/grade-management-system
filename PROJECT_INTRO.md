# 氹仔坊眾學校 — AI 成績管理系統

## 一句話介紹

> 老師用說的就能管理成績與德育——「查看初一甲數學排名」，AI 自動查詢、分析、產生表格與報告。

---

## 這是什麼？

一個為中學設計的 **AI 對話式成績與德育管理系統**。老師不需要學習任何軟體操作，只要像聊天一樣輸入需求，AI 就會自動完成工作。

---

## 為什麼需要這個？

### 傳統方式的痛點

| 痛點 | 說明 |
|------|------|
| 操作複雜 | 傳統系統要點很多層選單才能查一科成績 |
| 學習成本高 | 每個功能要記步驟，新老師要培訓 |
| 手工統計 | 排名、平均、不及格人數要自己用 Excel 算 |
| 容易出錯 | 複製貼上成績，漏貼或貼錯很難發現 |
| 資料分散 | 成績散落在不同 Excel 檔，找不到歷史記錄 |

### 我們的解法

**用對話取代操作。** 老師只需要「說」出需求，AI 負責理解意圖、查詢資料庫、計算統計、產生結果、生成報告。

---

## 能做什麼？

### 📊 成績查詢
```
「查看初一甲中文讀本的大測成績」
→ AI 查詢資料庫，即時顯示互動表格（可排序、分頁）
```

### 🏆 學生排名
```
「初一甲數學的學生排名」
→ 自動計算各學生平均分，按排名顯示
```

### ✏️ 成績登記
```
「幫初一甲數學登記一次小考，標題「第三次小考」，學生1號85分、2號92分」
→ 直接寫入資料庫，即時生效
```

### 📋 學期草榜
```
「產生初一甲的學期成績草榜」
→ 自動計算各科成績、總分、排名、不及格標記
```

### 🔍 異常偵測
```
「偵測初一甲中文讀本的成績異常」
→ AI 用統計方法自動標記分數異常偏高/偏低、驟降的學生
```

### 📝 補考建議
```
「初一甲有哪些學生需要補考」
→ 自動列出不及格學生及建議補考科目
```

### 📥📥 Excel 匯入匯出
```
「匯出初一甲中文讀本的成績 Excel」
「匯入這個檔案的成績」
→ 支援批次處理，不用一筆筆 key
```

### 📄 成績單 PDF
```
「產生初一甲的成績單 PDF」
→ 自動產生可列印的成績單
```

### 🎖️ 獎懲管理
```
「登記學生5號一個優點，原因：熱心服務」
→ 獎懲記錄直接寫入資料庫
```

### 📏 常規記錄
```
「查看初一甲的常規違紀統計」
→ 顯示欠作業、遲到、缺席等統計表格
```

### 📊 操行評估
```
「產生初一甲的操行評估草榜」
→ 自動計算抵銷前後的缺點/優點，產生操行等級
```

### 📈 德育分析（AI 深度報告）
```
「分析初一甲的違紀情況」
→ 產生違紀排名表、分布圖、類型統計圖 + AI 深度分析報告
  - 整體情況評估
  - 主要違紀類型分析
  - 特殊案例與關注名單
  - 改善建議與行動方案
  - 總結
```

### 📋 德育報表
```
「產生初一甲班級月報」
「產生學生5號的個人德育報告」
「產生獎懲統計報表」
「產生常規違紀報表」
→ 多種德育報表，支援匯出
```

---

## 跟傳統系統有什麼不同？

| | 傳統系統 | 本系統 |
|---|---------|--------|
| 操作方式 | 點選單、填表單 | 自然語言對話 |
| 學習成本 | 需要培訓 | 會說話就會用 |
| 查成績 | 5-10 次點擊 | 一句話 |
| 排名統計 | 自己開 Excel 算 | AI 自動計算 |
| 異常發現 | 靠老師肉眼 | AI 統計偵測 |
| 補考名單 | 逐科篩選 | 一鍵產生 |
| 草榜 | 手工拼湊 | 自動產生含排名 |
| 德育管理 | 分散在不同表 | AI 對話式管理 |
| 分析報告 | 無 | AI 自動生成深度報告 |

---

## 系統優勢

1. **零學習成本** — 會說話就會用，新老師無需培訓
2. **AI 意圖識別** — 自動判斷你想查成績、登記、分析還是匯出
3. **即時互動表格** — 查詢結果可排序、分頁，不用再貼到 Excel
4. **智能上下文** — AI 知道學校的班級、科目、學期，自動轉換名稱和 ID
5. **統計分析** — 排名、平均、標準差、異常偵測，AI 幫你算
6. **AI 深度報告** — 分析結果自動生成結構化報告，含現況評估、改善建議
7. **批次操作** — Excel 匯入匯出，大量成績一次處理
8. **資料安全** — AI 不編造數據，所有結果來自資料庫真實查詢
9. **權限控制** — 不同角色只能使用對應功能
10. **德育整合** — 獎懲、常規違紀、操行評估一體化管理
11. **多模型路由** — 支援 OpenAI / Anthropic / 本地模型，按任務類型自動選擇最適模型

---

## 目前測試數據

已匯入氹仔坊眾學校 2025-2026 學年初一甲班真實數據：

- 28 名學生
- 8 個科目（中文讀本、英文讀本、英文文法、數學、綜合科學、歷史、地理、公民教育）
- 10 次大測
- 280 筆成績記錄
- 28 筆常規記錄（欠作業、欠課本、上課違規、儀表不符、遲到、缺席、請假）
- 8 位老師

---

## 部署架構

| 組件 | URL | 說明 |
|------|-----|------|
| 前端 (Vercel) | https://grade-management-system-amber.vercel.app | React + TypeScript |
| 後端 (ngrok) | https://hazy-bamboo-skeleton.ngrok-free.dev | FastAPI + SQLAlchemy |
| 資料庫 | Supabase PostgreSQL | 雲端資料庫 |

---

## 登入帳號

| 帳號 | 密碼 | 姓名 | 角色 |
|------|------|------|------|
| admin | 123456 | 系統管理員 | admin |
| teacher1~8 | 123456 | 各科任老師 | teacher |

---

## 技術架構

- **後端：** Python FastAPI + PostgreSQL + SQLAlchemy + Alembic（資料庫迁移）
- **前端：** React + TypeScript + Vite
- **AI：** 多模型路由（OpenAI-compatible / Anthropic / Ollama）+ Function Calling
- **技能系統：** 20 個 AI 技能，按角色權限控制
- **部署：** Docker Compose（本地開發）+ Vercel（前端）+ ngrok（後端）

---

## AI 技能清單（20 個）

### 日常成績（4）
| 技能 | 說明 |
|------|------|
| `daily_grade.check` | 查詢日常成績 |
| `daily_grade.register` | 登記日常成績 |
| `daily_grade.analyze` | 分析日常成績 + AI 深度報告 |
| `daily_grade.report` | 產生日常成績報表 |

### 學期成績（3）
| 技能 | 說明 |
|------|------|
| `semester_grade.register` | 登記學期成績 |
| `semester_grade.check` | 查詢學期成績 |
| `draft_list` | 產生學期成績草榜 |

### 系統功能（2）
| 技能 | 說明 |
|------|------|
| `system.import_excel` | Excel 匯入 |
| `system.export_excel` | Excel 匯出 |

### 成績單（1）
| 技能 | 說明 |
|------|------|
| `transcript.generate` | 產生成績單 PDF |

### AI 增強分析（3）
| 技能 | 說明 |
|------|------|
| `ai_enhanced.anomaly_detect` | 成績異常偵測 |
| `ai_enhanced.class_comparison` | 班級對比分析 |
| `ai_enhanced.makeup_suggestion` | 補考建議 |

### 德育管理（7）
| 技能 | 說明 |
|------|------|
| `conduct.draft_list` | 常規記錄草榜 |
| `conduct.analysis` | 德育分析 + AI 深度報告（違紀排名表、分布圖、類型統計圖） |
| `conduct.assessment_input` | 操行評估輸入 |
| `conduct.class_monthly_report` | 班級月報 |
| `conduct.student_individual_report` | 學生個人德育報告 |
| `conduct.rewards_punishments_report` | 獎懲統計報表 |
| `conduct.regular_violations_report` | 常規違紀報表 |

---

## 資料模型

| 模型 | 說明 |
|------|------|
| School / Campus / AcademicYear / Semester | 學校組織架構 |
| Class / Student | 班級與學生 |
| Subject / ClassSubject | 科目與班級科目（含成績權重） |
| User | 使用者（admin / dept_head / teacher） |
| DailyGradeItem / DailyGrade | 日常成績項目與成績 |
| SemesterGrade / MakeupExam | 學期成績與補考 |
| DraftGradeList | 成績草榜 |
| AwardType / StudentAward | 獎項類型與學生獲獎 |
| GradeAnomaly | 成績異常記錄 |
| GradeAppeal | 成績申訴 |
| PassingLine | 及格分數線 |
| TableFormatTemplate | 表格格式模板 |
| RewardPunishment | 獎懲記錄 |
| RegularViolation | 常規違紀 |
| ConductAssessment | 操行評估 |

---

## 未來規劃

- [x] 多班級支援（初一甲乙丙丁...）- 架構已支援
- [ ] 家長端查詢（家長只能看自己孩子成績）
- [ ] 微信/短信通知家長成績
- [ ] 成績申訴流程完善
- [ ] 歷年成績趨勢分析
- [ ] 更多 AI 分析（學期預測）
- [ ] 成績趨勢分析技能（`trend_analysis`）
- [ ] 班級學期/學年回顧技能（`semester_review` / `year_review`）
- [ ] 光榮榜技能（`honor_roll`）
- [ ] 獎項登記技能（`award`）
- [ ] 及格線設定技能（`passing_line`）
- [ ] 申訴處理技能（`appeal_handle`）
- [ ] 通知技能（`notify`）
- [ ] 批量成績單產生（`batch_generate`）
- [ ] 學期成績分析技能（`semester_grade.analyze`）
- [ ] 結合成績與德育的綜合報告